...
*Fig. 2. Angled tip detail (side view at 60°) showing the tip zone geometry and the spatial relationship between the ink nib and the internal axial force sensing element.*

**Fig. 3** illustrates the FSR 402 integration mechanism. The FSR sensor sits directly above a pressure-loaded spring, with the ballpoint refill running through the center of the spring. The three inset panels demonstrate the spring state during in-air (uncompressed), in-air (compressed), and on-surface (normalized pressure signal) conditions, confirming successful mechanical isolation of axial writing force from grip pressure.

![Fig. 3 – FSR 402 integration with spring mechanism and on-surface/in-air state diagrams](fig5_fsr_architecture.png)

*Fig. 3. FSR 402 integration diagram showing the spring-loaded plunger mechanism and three operating states: in-air (spring uncompressed), in-air (spring compressed), and on-surface (normalized pressure signal). This architecture achieves mechanical decoupling of axial tip force from fingerprint grip pressure.*
...
*Fig. 4. The MPU6050 ITG/MPU breakout board. This 6-DOF MEMS IMU module is mounted internally near the pen tip to capture vibration and acceleration data in the 4–6 Hz Parkinsonian tremor range.*

* **Tri-FSR Circuit:** Two FSR-402 sensors (larger active area for fingerprint grip) and one FSR-400 (smaller active area for the internal tip). The FSR sensor used for grip sensing is shown in **Fig. 5**. Each is placed in a voltage divider circuit with a 10 kΩ pull-down resistor. A 100 nF capacitor forms a primary low-pass RC filter (~159 Hz cutoff) to eliminate high-frequency electrical noise. Additionally, to accommodate the ESP32-C3's specific ADC bootstrapping pins (GPIO 1, 2, and 3), a boot-safe high-impedance filter stage comprising a 100 kΩ series resistor and a 10 µF capacitor to GND is added. This isolates the pins during power-on and provides a secondary low-pass filter (~0.16 Hz cutoff) to highly stabilize the 4–6 Hz tremor baseline.
...
**Fig. 3.** FSR 402 integration diagram showing the spring-loaded plunger mechanism and three operating states (in-air uncompressed, in-air compressed, on-surface), confirming mechanical decoupling of axial tip force from fingerprint grip pressure. (`fig5_fsr_architecture.png`)

**Fig. 4.** The MPU6050 ITG/MPU breakout board used as the vibration and acceleration sensing module, mounted internally near the pen tip to capture 4–6 Hz tremor signatures. (`fig4_mpu6050.jpg`)

**Fig. 5.** The FSR-402 force-sensitive resistor used in the dual-sensor fingerprint grip array to capture mean grip pressure and high-frequency micro-tremor oscillations. (`fig3_fsr_sensor.jpg`)
